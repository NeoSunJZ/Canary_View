<template>
  <div :id="chartID" style="height: 300px;width: 500px;"></div>
</template>
...
import { getQuickTaskConfigByConfigID } from '@/api/config-api/quickTaskConfig.js'
import 'echarts-gl';
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const GenNonDuplicateID = () => {
      return "configTransferTestMap" + Math.random().toString();
    }
    onBeforeMount(() => {
      chartID.value = GenNonDuplicateID();
    });

    onMounted(() => {
      let chartDom = document.getElementById(chartID.value);
    ...
    const loadInintData = async () => {
      ...
      configInfo.value = await getQuickTaskConfigByConfigID(configID.value);
      taskConfig.value = JSON.parse(configInfo.value['config']);
