...
<template>
  <a-spin tip="正在加载" :spinning="loading">
    <a-card size="small" v-if="inferenceResultDataset[imgID]!=null" :title="'图片 '+imgID+' 在 '+modelName+' 上的预测结果'">
      ...
      <a-tabs v-model:activeKey="advImgInfoKey" @change="loadAdvInfo">
        <a-tab-pane v-for="(data,key) in inferenceResultDataset[imgID]['attack']" :key="key" :tab="key">
          ...
          <a-descriptions :column="3" bordered size="small" v-if="advInfoList[data.advImgID]!=null">
            <a-descriptions-item label="生成耗时">{{parseFloat(advInfoList[data.advImgID]['cost_time']).toFixed(5)}} 秒(s)</a-descriptions-item>
            <a-descriptions-item label="最大像素扰动">
              {{parseFloat(advInfoList[data.advImgID]['adv_img_maximum_disturbance']).toFixed(7)}}
            </a-descriptions-item>
            <a-descriptions-item label="像素平均欧式距离">
              {{parseFloat(advInfoList[data.advImgID]['adv_img_euclidean_distortion']).toFixed(10)}}
            </a-descriptions-item>
            <a-descriptions-item label="像素变化率">
              {{parseFloat(advInfoList[data.advImgID]['adv_img_pixel_change_ratio']).toFixed(5)*100 +"%"}}
            </a-descriptions-item>
            <a-descriptions-item label="深度特征相似性">
              {{parseFloat(advInfoList[data.advImgID]['adv_img_deep_metrics_similarity']).toFixed(7)}}
            </a-descriptions-item>
            <a-descriptions-item label="结构特征相似性">
              {{parseFloat(advInfoList[data.advImgID]['adv_img_low_level_metrics_similarity']).toFixed(7)}}
            </a-descriptions-item>
          </a-descriptions>
          <a-row :gutter="10">
            <a-col :span="16">
            ...
            <a-col :span="8">
              ...
              <a-image v-if="advInfoList[data.advImgID]!=null" :src="advInfoList[data.advImgID]['adv_img']['diff']" style="width:100%;padding:10px" />
            </a-col>
          </a-row>
        </a-tab-pane>
      </a-tabs>
